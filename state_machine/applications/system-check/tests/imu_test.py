...
def user_test(test_type):
    ...
    if test_type == "Still":
        ...
        # collect acceleration and gyroscope readings, return 0.0 for mag as default
        time.sleep(wait_time)
        ...
        # at some random time, collect acceleration and gyroscope readings, return 0.0 for mag
        ...
        # print user prompts
        print("Please rotate the cubesat as best as possible. Waiting", wait_time, "seconds.")
        time.sleep(wait_time)
        print("Collecting IMU data...")

        # collect acceleration and gyroscope readings, return 0.0 for mag
        time.sleep(wait_time)
        acc = cubesat.acceleration()
        gyro = cubesat.gyro()
        print("Data Collection Complete")
        ...
        time.sleep(wait_time)
        print("Please slowly move the magnet closer to the cubesat for", wait_time, "seconds")
        print("Collecting IMU data...")
...
def imu_test(result_dict, test_type):
    ...
    if test_type == "Still":
        ...
        result_val_string_acc = "Testing IMU when cubesat is still. Acc: " + str(acc)
        result_val_string_gyro = "Testing IMU when cubesat is still. Gyro: " + str(gyro)

        # if total acceleration is within 0.2 m/s^2 of 9.8 and gyro ~= 0 deg/s, true
        result_val_bool_acc = abs(9.8 - sqrt(acc[0] ** 2 + acc[1] ** 2 + acc[2] ** 2)) < 0.2
        result_val_bool_gyro = sqrt(gyro[0] ** 2 + gyro[1] ** 2 + gyro[2] ** 2) < 0.2
        result_dict['IMU_AccStill'] = (result_val_string_acc, result_val_bool_acc)
        result_dict['IMU_GyroStill'] = (result_val_string_gyro, result_val_bool_gyro)
        ...
        result_val_bool = abs(acc[0]) > 0 and abs(acc[1]) > 0 and abs(acc[2]) > 0
        result_dict['IMU_AccMoving'] = (result_val_string, result_val_bool)

    elif test_type == "Rotating":
        # record gyro reading
        acc, gyro, mag = user_test(test_type)
        result_val_string = "Testing IMU when cubesat is rotating. Gyro: " + str(gyro)
        ...
        # collect temperature reading and ask user to check a thermostat and confirm
        ...
        result_dict['IMU_Temp'] = ("IMU Temperature Reading: " + str(temp), result_val_bool)
...
def run(hardware_dict, result_dict):
    ...
    if not hardware_dict['IMU']:
        result_dict['IMU_AccStill'] = ('Cannot test accelerometer; no IMU detected', False)
        result_dict['IMU_AccMoving'] = ('Cannot test accelerometer; no IMU detected', False)
        result_dict['IMU_GyroStill'] = ('Cannot test gyroscope; no IMU detected', False)
        result_dict['IMU_GyroRotating'] = ('Cannot test gyroscope; no IMU detected', False)
        result_dict['IMU_MagMagnet'] = ('Cannot test magnetometer; no IMU detected', False)
